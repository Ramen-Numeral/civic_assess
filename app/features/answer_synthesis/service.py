...
class AnswerSynthesisService:
    ...
    async def audit(
        ...
        try:
            ...
            raise InvalidAnswerProposalError(
                "Answer audit violated paragraph alignment"
            ) from exc

    async def _write(
        self,
        request: GroundedAnswerRequest,
        draft: NaturalAnswerDraft | None = None,
        ...
        for attempt in range(2):
            try:
                ...
                return NaturalAnswerDraft(
                    paragraphs=tuple(
                        AnswerParagraph(
                            paragraph_id=uuid4(),
                            text=text.strip(),
                            finding_indexes=tuple(
                                findings[ref] for ref in item.finding_refs
                            ),
            ...
            if attempt:
                LOGGER.error(
                    ...
                    extra={
                        "event": "answer.writer.contract_repair_exhausted",
            ...
            LOGGER.warning(
                ...
                extra={
                    "event": "answer.writer.contract_repair",
                    "error_type": type(failure).__name__,
                    "error_message": str(failure),
                },
            )
    ...
    def _raise_model_error(exc: LLMError, stage: str) -> None:
        if exc.failures and all(
            f.kind is FailureKind.INVALID_OUTPUT for f in exc.failures
        ):
            raise InvalidAnswerProposalError(
...
def _strip_refs(value: str) -> str:
    return " ".join(PARENTHETICAL_REF.sub("", BRACKETED_REF_RUN.sub("", value)).split())
    ...
    return (
        {
            ...
            "findings": [
                {
                    "ref": ref,
                    "statement": request.findings[index].statement,
                    "evidence_refs": [
                        evidence_refs[c]
